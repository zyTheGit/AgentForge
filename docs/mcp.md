...
- **pi**（`.pi\mcp.json` 的 `mcpServers`，需 `pi-mcp-adapter`）：条目**没有 `type` 字段**，适配器按 `command` / `url` / `socket` 互斥来判定 transport。远端条目**两种 transport 都显式写** `httpTransport`：`sse` 写 `"sse"`（锁定 SSE，所以 SSE 在 pi 上无损），`http` 写 `"streamable-http"`。顶层键名与 Claude Code 同名，但条目形状**不同构**，别照抄。
  ...
  - **`http` 为什么不留空**（issue [#69](https://github.com/zyTheGit/AgentForge/issues/69)）：`.pi\mcp.json` 走 `merge_json` + `soft`（保住用户手工加的 server），而深合并**删不掉**本轮不再产出的键。若 `http` 留空，把某个 server 的 `transport` 从 `sse` 改回 `http` 再 sync，上一轮的 `httpTransport: "sse"` 会留在文件里——声明已改、pi 却仍锁在 SSE，且 `sync` 报 unchanged、`doctor` 也看不出异常。显式写取值让声明与产物一一对应。
  - **代价：`http` 不再自动回落 SSE。** 适配器只在该键为 `undefined` 时才走「先 POST、被拒后回落 SSE」，写了取值就直接按取值连。于是 `transport: http` 的端点若只会说 SSE，会连接失败而不是被静默救回——想要 SSE 就把 SoT 里的 `transport` 声明成 `sse`，不靠运行时猜。
