...
| 会覆盖内置模板吗 | 不会。内置的 `base/default` / `base/tools` / `base/context` 恒优先（见 [templates/ 模板](templates.md#三个内置模板)），启用官方源只**新增**它独有的模板 id（同名 id 会在 `template list` 里各列一行，渲染时仍取内置那份） |
