...
- **transport 能力矩阵**。矩阵只装四个内置 target 的实测结论。声明它了又声明了 MCP server 时，`aforge sync` 与 `aforge doctor` 各出**每 target 一条**（不是每 server 一条）的 `mcp-transport/<id>-unmeasured` warn：投影照常，只是「该 target 对 stdio / http / sse 到底支持到什么程度」没有实测依据，不猜默认值。warn 不影响退出码。
