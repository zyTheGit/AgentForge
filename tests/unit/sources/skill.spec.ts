...
describe('addSkill', () => {
  it('--from 源根路径：copy 到目标层 skills/<name>/，实体副本（改源不影响已安装副本）', async () => {
    ...
    expect(host.files.get(path.win32.join(PROJECT_SOT, 'skills', 'pdf', 'SKILL.md'))).toBe(
      'V1 说明',
    );
  ...
  it('递归 copy：源 skill 含子目录与多文件 → 全部复制（相对路径保留）', async () => {
    ...
    expect([...result.files].sort()).toEqual(
      ['SKILL.md', 'assets\\deep\\x.md', 'assets\\intro.md'].sort(),
    );
    expect(
      host.files.get(path.win32.join(PROJECT_SOT, 'skills', 'big', 'assets', 'deep', 'x.md')),
    ).toBe('deep');
  });
...
describe('addSkill 安全边界：symlink / 环路 / 深度', () => {
  it('symlink 项被跳过且不读取内容，作为 skipped 返回（不跟随到 ~/.ssh/id_rsa 之类目标）', async () => {
    ...
    const skillDir = path.win32.join(VENDOR, 'skills', 'pdf');
    base.files.set(path.win32.join(skillDir, 'SKILL.md'), '正文');
    // 恶意源里的链接项（内容模拟"跟随后会读到的私钥"）
    const link = path.win32.join(skillDir, 'id_rsa-link');
    ...
    expect(host.files.has(path.win32.join(PROJECT_SOT, 'skills', 'pdf', 'id_rsa-link'))).toBe(
      false,
    );
  });
  ...
  it('目录型 symlink（Windows junction 同）被跳过，不递归进去', async () => {
    ...
    const skillDir = path.win32.join(VENDOR, 'skills', 'pdf');
    base.files.set(path.win32.join(skillDir, 'SKILL.md'), '正文');
    const linkDir = path.win32.join(skillDir, 'loop');
    base.files.set(path.win32.join(linkDir, 'inner.md'), '不该被复制');
    const host = withSymlinks(base, [linkDir]);

    const result = await addSkill(skillCtx(host), 'pdf', VENDOR);
    expect(result.files).toEqual(['SKILL.md']);
    expect(result.skipped).toEqual([{ path: linkDir, reason: 'symlink' }]);
    expect(host.files.has(path.win32.join(PROJECT_SOT, 'skills', 'pdf', 'loop', 'inner.md'))).toBe(
      false,
    );
  });
  ...
  it('环路基准：子项解析回已访问目录（非 symlink 路径）→ 记为 cycle 并跳过', async () => {
    const base = createDirAwareHost();
    const skillDir = path.win32.join(VENDOR, 'skills', 'pdf');
    const sub = path.win32.join(skillDir, 'sub');
    base.files.set(path.win32.join(skillDir, 'SKILL.md'), '正文');
    base.files.set(path.win32.join(sub, 'a.md'), 'a');
    ...
    const result = await addSkill(skillCtx(host), 'pdf', VENDOR);
    expect(result.files.sort()).toEqual(['SKILL.md', 'sub\\a.md']);
  ...
  it(`深度超过 ${MAX_COPY_DEPTH} 层 → ConfigError(2)，并回滚本次新建的目标目录`, async () => {
    ...
    host.files.set(
      path.win32.join(VENDOR, 'skills', 'deep', ...segments, 'SKILL.md'),
      '深处的文件',
    );

    await expect(addSkill(skillCtx(host), 'deep', VENDOR)).rejects.toMatchObject({ code: 2 });
    const targetPrefix = path.win32.join(PROJECT_SOT, 'skills', 'deep');
...
describe('addSkill 失败清理（不留残骸挡死下次安装）', () => {
  ...
  it('copy 中途失败 → 目标目录整体回滚，重试 skill add 不再被 ConflictError(3) 拦死', async () => {
    ...
    const skillDir = path.win32.join(VENDOR, 'skills', 'pdf');
    host.files.set(path.win32.join(skillDir, 'a.md'), 'a');
    host.files.set(path.win32.join(skillDir, 'b.md'), 'b');
    host.files.set(path.win32.join(skillDir, 'SKILL.md'), 'doc');

    await expect(addSkill(skillCtx(host), 'pdf', VENDOR)).rejects.toThrow(/模拟写入失败/);
    const targetPrefix = path.win32.join(PROJECT_SOT, 'skills', 'pdf');
    expect([...host.files.keys()].filter((k) => k.startsWith(targetPrefix))).toEqual([]);
  ...
  it('目标目录已存在但为空：失败仍清空内容（冲突判据与回滚判据同源）', async () => {
    ...
    const targetDir = path.win32.join(PROJECT_SOT, 'skills', 'pdf');
