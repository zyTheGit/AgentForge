...
import { promoteLearning, type PromoteResult } from '../core/learning/promote';
...
export async function runPromote(
  ...
  return promoteLearning(
    { host: ctx.host, env: readEnv(ctx.host), os: ctx.os, cwd: ctx.cwd },
    id,
    { to: options.to },
  );
}
...
export function registerPromoteCommand(program: Command): void {
  program
    ...
    .action(async (id: string, options: { to?: string; yes?: boolean }) => {
      ...
      const result = await runPromote({ host: realHost, cwd: process.cwd(), os: currentOs() }, id, {
        to,
      });
