 ...
 * - `sources.json` / `store\`：源登记表与 git 源缓存**恒在 user 层**（§3.1）。用户在任意
 *   项目里管过一次源（`aforge source add` / `source enable official`）都会建出 user 层的
 *   `sources.json`，此后 `aforge init --scope user`
