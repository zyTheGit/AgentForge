 ...
 * （禁用 → 跳过；启用但没缓存 → 只是它的模板 id 解析不到，那由 §9 第 5 条
 * `template/<id>` 那项负责报 error）。让本项抬高退出码，等于让"没启用官方源"
 * 这件正常状态把 CI 里的 `aforge doctor` 弄红。
 *
...
async function describeDefaultSource(
  ...
  if (registered === undefined) {
    return {
      ...
      level: 'ok',
      item,
      detail: `${decl.description}：未登记（本机 sources.json 早于该特性，或已被 aforge source remove）`,
  ...
  const commit = registered.type === 'git' ? (registered.commit ?? '(未记录)') : '-';
  return {
    section: 'config',
    level: 'ok',
    item,
